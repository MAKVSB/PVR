    ...
    match event {
        ...
        Event::PassTime(time) => match computer {
            ...
            ComputerState::Running { uptime, idle_time } => {
                match idle_time + time {
                    ..1000 => ComputerState::Running {
                        uptime: uptime + time,
                        idle_time: idle_time + time,
                    },
                    1000..1500 => ComputerState::Sleeping {
                        uptime: uptime + time,
                        sleep_time: idle_time + time - 1000,
                    },
                    1500.. => ComputerState::Off,
                }
            }
            ComputerState::Sleeping { uptime, sleep_time } => {
                match sleep_time + time {
                    ..500 => ComputerState::Sleeping {
                        uptime: uptime + time,
                        sleep_time: sleep_time + time,
                    },
                    500.. => ComputerState::Off,
                }
            }
        },
