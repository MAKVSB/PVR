...
impl MemoryMap {
    ...
    fn read(&self, address: Address, count: usize) -> Option<Vec<u8>> {
        ...
        while remaining > 0 {
            let (start, buffer) = self.map.range(..=current_address).next_back()?;
            let offset = current_address.saturating_sub(*start);
            if offset >= buffer.len() {
                return None; // Missing bytes in address range
            }
            let available = buffer.len() - offset;
            let to_read = available.min(remaining);

            result.extend_from_slice(&buffer[offset..offset + to_read]);
    ...
    fn write(&mut self, address: Address, buffer: Vec<u8>) {
        let mut new_start = address;
        let mut new_buffer = buffer;

        //Left neigh
        //No need to loop here since you can hit collision with left neighbor only once :D WEEEEEEE
        if let Some((&left_start, left_buffer)) = self.map.clone().range(..new_start).next_back() {
            if left_start + left_buffer.len() >= new_start {
                let mut merged_buffer = left_buffer.clone();
                if left_start + left_buffer.len() > new_start {
                    merged_buffer.truncate(new_start - left_start);
                }
                merged_buffer.extend(new_buffer);
                new_buffer = merged_buffer;
        ...
        // Right neigh
        loop {
            let next_in_tree = self.map.range(new_start..).next();
            match next_in_tree {
                Some((&right_start, right_buffer)) => {
                    if new_start + new_buffer.len() >= right_start {
                        let mut right_clone = right_buffer.clone();
                        if new_start + new_buffer.len() > right_start {
                            let elem_to_remove = new_start + new_buffer.len() - right_start;
                            if elem_to_remove > right_buffer.len() {
                                //PROBLEM -> we need to remove whole and start looping to check for next in tree
                                self.map.remove(&right_start);
                                continue;
                            }
                            right_clone.drain(0..elem_to_remove);
                        }
                        new_buffer.extend(right_clone);
                        self.map.remove(&right_start);
                    } else {
                        break;
                    }
        ...
        self.map.insert(new_start, new_buffer);
    }
