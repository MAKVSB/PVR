...
fn parse_day(day: &str) -> Option<Day> {
...
fn parse_ticket(ticket_string: &str) -> Option<Ticket<'_>> {
    let mut parts = ticket_string.split(';');

    let movie = match parts.next() {
        Some(movie) => {
            match movie.chars().all(|c| c.is_ascii_alphanumeric() || c == ' ') {
                false => return None,
                true => movie,
            }
        }
    ...
    let day = parse_day(parts.next()?)?;
    let visitor = match parts.next() {
        Some(visitor) => {
            match visitor.chars().all(|c| c.is_ascii_alphanumeric() || c == ' ') {
                false => return None,
                true => {
                    match visitor {
                        "" => None,
