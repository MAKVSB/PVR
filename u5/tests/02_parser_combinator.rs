...
fn take_while<F>(pred: F) -> impl FnMut(&str) -> ParseResult<&str, String>
where
    F: Fn(char) -> bool,
{
    move |input: &str| {
        let parsed: String = input.chars().take_while(|&c| pred(c)).collect();
        let rest = &input[parsed.len()..];
        Ok((rest, parsed))
    }
}

fn string_parser(expected: & str) -> impl FnMut(&str) -> ParseResult<&str, String> + '_ {
    move |input: &str| {
        if let Some(rest) = input.strip_prefix(expected) {
            Ok((rest, expected.to_string())) // Convert to String for the output
        } else {
            Err(format!("Expected '{}', but got '{}'", expected, input))
        }
    }
}

fn map<P, F, A, B>(mut parser: P, f: F) -> impl FnMut(&str) -> ParseResult<&str, B>
where
    P: Parser<A>,
    F: Fn(A) -> B,
{
...
fn try_map<P, F, A, B>(mut parser: P, f: F) -> impl FnMut(&str) -> ParseResult<&str, B>
where
    P: Parser<A>,
...
fn or<P1, P2, A>(mut p1: P1, mut p2: P2) -> impl FnMut(&str) -> ParseResult<&str, A>
where
    P1: Parser<A>,
    P2: Parser<A>,
{
    move |input: &str| p1.parse(input).or_else(|_| p2.parse(input))
}

fn opt<P, A>(mut parser: P) -> impl FnMut(&str) -> ParseResult<&str, Option<A>>
where
    P: Parser<A>,
{
...
fn preceded_by<P1, P2, A, B>(mut p1: P1, mut p2: P2) -> impl FnMut(&str) -> ParseResult<&str, B>
where
    P1: Parser<A>,
    P2: Parser<B>,
{
    move |input: &str| {
        let (rest, _) = p1.parse(input)?;
        p2.parse(rest)
    }
}

fn followed_by<P1, P2, A, B>(mut p1: P1, mut p2: P2) -> impl FnMut(&str) -> ParseResult<&str, A>
where
    P1: Parser<A>,
    P2: Parser<B>,
{
    move |input: &str| {
        let (rest, value) = p1.parse(input)?;
        p2.parse(rest).map(|(next_rest, _)| (next_rest, value))
    }
}

fn delimited_by<A, P>(start: char, end: char, parser: P) -> impl FnMut(&str) -> ParseResult<&str, A>
where
    P: Parser<A>,
{
    preceded_by(char_parser(start), followed_by(parser, char_parser(end)))
}

fn tag_parser(tag: &str) -> impl FnMut(&str) -> ParseResult<&str, String> {
    let open_tag = format!("<{}>", tag);
    let close_tag = format!("</{}>", tag);

    move |input: &str| {
        preceded_by(
            string_parser(&open_tag),
            followed_by(take_while(|c| c != '<'), string_parser(&close_tag)),
        )(input)
    }
}

fn sequence<P1, P2, A, B>(mut p1: P1, mut p2: P2) -> impl FnMut(&str) -> ParseResult<&str, (A, B)>
where
    P1: Parser<A>,
    P2: Parser<B>,
{
    move |input: &str| {
        let (rest, result1) = p1.parse(input)?;
        let (rest2, result2) = p2.parse(rest)?;

        Ok((rest2, (result1, result2)))
    }
}

fn repeated<P, A>(mut parser: P) -> impl FnMut(&str) -> ParseResult<&str, Vec<A>>
where
    P: Parser<A>,
{
    move |mut input: &str| {
